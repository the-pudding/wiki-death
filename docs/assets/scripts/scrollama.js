(function (global, factory) {
	...
	function select(selector) {
		...
		else if (typeof selector === 'string') { return document.querySelector(selector); }
		return null;
	}
	...
	function scrollama() {
		...
		function handleResize() {
			...
			if (debugMode) { update({ id: id, stepOffsetHeight: stepOffsetHeight, offsetMargin: offsetMargin, offsetVal: offsetVal }); }
		}
		...
		function notifyStepEnter(element, direction, check) {
			...
			if (preserveOrder && check && direction === 'down') { notifyOthers(index, 'above'); }
		...
		function notifyStepProgress(element, progress) {
			...
			if (callback.stepProgress && typeof callback.stepProgress === 'function') { callback.stepProgress(resp); }
		}
		...
		function notifyContainerExit() {
			...
			if (callback.containerExit && typeof callback.containerExit === 'function') { callback.containerExit(resp); }
		}
		...
		function intersectStepAbove(entries) {
			...
			entries.forEach(function (entry) {
				...
				if (bottomAdjusted >= -ZERO_MOE) {
					if (isIntersecting && direction === 'down' && ss.state !== 'enter') { notifyStepEnter(target, direction); }
					else if (!isIntersecting && direction === 'up' && ss.state === 'enter') { notifyStepExit(target, direction); }
					else if (
						!isIntersecting &&
